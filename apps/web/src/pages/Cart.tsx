...
import type { CartData } from '../types/domain'
...
export default function Cart() {
  ...
  const handleUpdateQuantity = async (variantId: string | number, quantity: number) => {
    try {
  ...
  const handleRemove = async (variantId: string | number) => {
    try {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-5xl mx-auto px-6 py-12">
        ...
        <h1 className="text-3xl font-bold mb-8" style={{ color: 'var(--brand-primary)' }}>Your cart</h1>
        ...
        {loading ? (
          ...
          <div className="text-center py-24">
            ...
            <h2 className="text-2xl font-bold mb-3" style={{ color: 'var(--brand-primary)' }}>Your cart is empty</h2>
            <p className="text-gray-500 mb-8">Add some products to get started</p>
            <Link to="/products" style={{ backgroundColor: 'var(--brand-accent)' }} className="px-8 py-3 rounded-xl text-white font-semibold hover:opacity-90 transition-opacity">
              Browse products
            </Link>
          </div>
        ) : (
              ...
              {cart.items.map(item => (
                <div key={item.variantId} className="bg-white rounded-2xl border border-gray-200 p-5 flex gap-4 items-center hover:border-orange-200 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-sm" style={{ color: 'var(--brand-primary)' }}>{item.productName}</h3>
                    <p className="text-gray-400 text-xs mt-0.5">{item.size}</p>
                    <p className="text-sm font-bold mt-1" style={{ color: 'var(--brand-accent)' }}>{formatCurrency(item.price)} each</p>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="w-8 text-center font-semibold text-sm" style={{ color: 'var(--brand-primary)' }}>{item.quantity}</span>
                    <button
                      onClick={() => handleUpdateQuantity(item.variantId, item.quantity + 1)}
                      className="w-8 h-8 rounded-lg border border-gray-200 flex items-center justify-center text-gray-500 hover:border-orange-400 hover:text-orange-500 transition-all font-bold"
                    >
                  ...
                  <div className="text-right shrink-0">
                    <p className="font-bold text-sm" style={{ color: 'var(--brand-primary)' }}>{formatCurrency(item.subtotal)}</p>
                    <button
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-6 sticky top-6">
                ...
                <div className="space-y-3 mb-6">
                  {cart.items.map(item => (
                    <div key={item.variantId} className="flex justify-between text-sm">
                      ...
                      <span className="font-medium shrink-0" style={{ color: 'var(--brand-primary)' }}>{formatCurrency(item.subtotal)}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="border-t border-gray-100 pt-4 mb-6">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-xl" style={{ color: 'var(--brand-accent)' }}>{formatCurrency(cart.total)}</span>
                  </div>
                </div>
