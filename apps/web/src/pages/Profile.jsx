          ...
          {editingName ? (
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              ...
              <div>
                ...
                <p className="text-sm font-medium" style={{ color: 'var(--brand-primary)' }}>{user?.email}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 mb-1">Account type</p>
                <p className="text-sm font-medium capitalize" style={{ color: 'var(--brand-primary)' }}>{user?.userType}</p>
              </div>
            </div>
          )}
              ...
              {orders.map(order => (
                <div key={order.orderId} className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4">
                      <span className="font-bold" style={{ color: 'var(--brand-accent)' }}>{formatCurrency(order.total)}</span>
                    ...
                    <div className="border-t border-gray-100 p-5 space-y-3">
                      {order.items.map((item, i) => (
                        <div key={i} className="flex justify-between text-sm">
                          <div>
                            <p className="font-medium" style={{ color: 'var(--brand-primary)' }}>{item.productName}</p>
                            <p className="text-gray-400 text-xs mt-0.5">{item.size} × {item.quantity}</p>
                          </div>
                          <span className="font-medium" style={{ color: 'var(--brand-primary)' }}>{formatCurrency(item.subtotal)}</span>
                        </div>
                      ))}
                      <div className="border-t border-gray-100 pt-3 flex justify-between">
                        <span className="font-bold text-sm" style={{ color: 'var(--brand-primary)' }}>Total</span>
                        <span className="font-bold text-sm" style={{ color: 'var(--brand-accent)' }}>{formatCurrency(order.total)}</span>
                      </div>
                    </div>
                  )}
                </div>
