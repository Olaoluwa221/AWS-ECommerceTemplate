...
import type { Order, OrderStatus, ProfileUser } from '../types/domain'
...
const getStatusKey = (status?: OrderStatus): OrderStatus => status ?? 'pending'
  ...
  useEffect(() => {
    const fetchMe = async () => {
      try {
        ...
        const profileUser = (res.data ?? {}) as ProfileUser & {
          marketingOptIn?: boolean
          firstName?: string
          lastName?: string
        }

        setMarketingOptIn(Boolean(profileUser.marketingOptIn))
        setNameForm({
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="flex items-center justify-between mb-10">
          <div>
            <h1 className="text-3xl font-bold" style={{ color: 'var(--brand-primary)' }}>My profile</h1>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-8">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold" style={{ color: 'var(--brand-primary)' }}>Account details</h2>
          ...
          {editingName ? (
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <p className="text-sm font-medium" style={{ color: 'var(--brand-primary)' }}>{nameForm.firstName || '—'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 mb-1">Last name</p>
                <p className="text-sm font-medium" style={{ color: 'var(--brand-primary)' }}>{nameForm.lastName || '—'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 mb-1">Email</p>
                <p className="text-sm font-medium" style={{ color: 'var(--brand-primary)' }}>{user?.email ?? 'Not available'}</p>
              </div>
              <div>
          ...
          {!editingName && (
            <div className="border-t border-gray-100 pt-4 mt-4">
              {editingPassword ? (
                <div className="space-y-4">
                  <h3 className="text-sm font-semibold" style={{ color: 'var(--brand-primary)' }}>Change password</h3>
                  <div>
                    <label className="block text-sm font-medium mb-1" style={{ color: 'var(--brand-primary)' }}>Current password</label>
                    <input
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-1" style={{ color: 'var(--brand-primary)' }}>New password</label>
                    <input
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-1" style={{ color: 'var(--brand-primary)' }}>Confirm new password</label>
                    <input
        ...
        {/* Email preferences */}
        <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-8">
          <h2 className="text-lg font-bold mb-4" style={{ color: 'var(--brand-primary)' }}>Email preferences</h2>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium" style={{ color: 'var(--brand-primary)' }}>Marketing emails</p>
            ...
            <button
              ...
              style={{
                ...
                height: '28px',
                borderRadius: '14px',
                backgroundColor: marketingOptIn ? 'var(--brand-accent)' : '#D1D5DB',
                border: 'none',
        ...
        {/* Order history */}
        <div>
          <h2 className="text-xl font-bold mb-4" style={{ color: 'var(--brand-primary)' }}>Order history</h2>
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-16 text-center">
              ...
              <h3 className="text-lg font-semibold mb-2" style={{ color: 'var(--brand-primary)' }}>No orders yet</h3>
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="text-left">
                        <p className="font-bold text-sm" style={{ color: 'var(--brand-primary)' }}>Order #{order.orderId}</p>
                        <p className="text-xs text-gray-400 mt-0.5">{getDisplayDate(order.dateOrdered)}</p>
                      </div>
                      <span className={`text-xs font-medium px-2.5 py-1 rounded-full border capitalize ${statusColors[getStatusKey(order.orderStatus)]}`}>
                        {getStatusKey(order.orderStatus)}
                      </span>
                    </div>
                    <div className="flex items-center gap-4">
                      <span className="font-bold" style={{ color: 'var(--brand-accent)' }}>{formatCurrency(Number(order.total ?? 0))}</span>
                  ...
                  {expandedOrder === order.orderId && (
                    <div className="border-t border-gray-100 p-5 space-y-3">
                      {(order.items ?? []).map((item, i) => {
                        ...
                        const itemSubtotal = item.subtotal ?? (typeof item.price === 'number' && typeof item.quantity === 'number' ? item.price * item.quantity : 0)
                        return (
                          <div key={i} className="flex justify-between text-sm">
                            <div>
                              <p className="font-medium" style={{ color: 'var(--brand-primary)' }}>{itemName}</p>
                              <p className="text-gray-400 text-xs mt-0.5">{itemSize} × {item.quantity ?? 1}</p>
                            </div>
                      ...
                      <div className="border-t border-gray-100 pt-3 flex justify-between">
                        <span className="font-bold text-sm" style={{ color: 'var(--brand-primary)' }}>Total</span>
                        <span className="font-bold text-sm" style={{ color: 'var(--brand-accent)' }}>{formatCurrency(Number(order.total ?? 0))}</span>
                      </div>
                    </div>
                  )}
                </div>
