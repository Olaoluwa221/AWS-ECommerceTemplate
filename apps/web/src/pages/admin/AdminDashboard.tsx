...
export default function AdminDashboard() {
  ...
  const getOrderStatus = (status?: OrderStatus): OrderStatus => status ?? 'pending'
  const getDateValue = (value?: string) => value ? new Date(value).getTime() : 0

  const totalRevenue = orders
    .filter((o) => getOrderStatus(o.orderStatus) !== 'cancelled')
    .reduce((sum, o) => sum + Number(o.total ?? 0), 0)

  const pendingOrders = orders.filter((o) => getOrderStatus(o.orderStatus) === 'pending').length
  const recentOrders = [...orders]
    .sort((a, b) => getDateValue(b.dateOrdered) - getDateValue(a.dateOrdered))
    .slice(0, 5)
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold" style={{ color: 'var(--brand-primary)' }}>Dashboard</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-200 overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h2 className="font-bold" style={{ color: 'var(--brand-primary)' }}>Recent orders</h2>
              <Link to="/admin/orders" className="text-sm hover:opacity-80 transition-opacity font-medium" style={{ color: 'var(--brand-accent)' }}>
                View all →
              </Link>
            </div>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {recentOrders.map((order, i) => (
                    ...
                    >
                      <td className="px-6 py-4 text-sm font-medium" style={{ color: 'var(--brand-primary)' }}>#{order.orderId}</td>
                      <td className="px-6 py-4 text-sm text-gray-500">{order.dateOrdered ? new Date(order.dateOrdered).toLocaleDateString() : '—'}</td>
                      <td className="px-6 py-4 text-sm font-semibold" style={{ color: 'var(--brand-accent)' }}>{formatCurrency(Number(order.total ?? 0))}</td>
                      <td className="px-6 py-4">
                        <span className={`text-xs font-medium px-2.5 py-1 rounded-full border capitalize ${statusColors[getOrderStatus(order.orderStatus)]}`}>
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-2xl border border-gray-200 p-6">
              <h2 className="font-bold mb-4" style={{ color: 'var(--brand-primary)' }}>Quick actions</h2>
              <div className="space-y-3">
                ...
                >
                  ...
                  <div>
                    <p className="text-sm font-medium group-hover:text-orange-500 transition-colors" style={{ color: 'var(--brand-primary)' }}>Manage products</p>
                ...
                >
                  <span className="text-2xl">📋</span>
                  <div>
                    <p className="text-sm font-medium group-hover:text-orange-500 transition-colors" style={{ color: 'var(--brand-primary)' }}>Manage orders</p>
                ...
                >
                  ...
                  <div>
                    <p className="text-sm font-medium group-hover:text-orange-500 transition-colors" style={{ color: 'var(--brand-primary)' }}>View storefront</p>
            ...
            {/* Order breakdown */}
            <div className="bg-white rounded-2xl border border-gray-200 p-6">
              <h2 className="font-bold mb-4" style={{ color: 'var(--brand-primary)' }}>Order breakdown</h2>
              {loading ? (
                    ...
                    return (
                      <div key={status}>
                        <div className="flex justify-between text-xs mb-1">
                          ...
                          <span className="font-medium" style={{ color: 'var(--brand-primary)' }}>{count}</span>
                        </div>
