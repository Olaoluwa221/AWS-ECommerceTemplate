  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold" style={{ color: 'var(--brand-primary)' }}>Email campaigns</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-6">
            <h2 className="text-lg font-bold mb-6" style={{ color: 'var(--brand-primary)' }}>Compose</h2>

            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1" style={{ color: 'var(--brand-primary)' }}>Subject</label>
                <input
                  type="text"
                  value={form.subject}
                  onChange={e => setForm({ ...form, subject: e.target.value })}
                  placeholder={`e.g. ${siteConfig.marketing.exampleSubject}`}
              ...
              <div>
                <label className="block text-sm font-medium mb-1" style={{ color: 'var(--brand-primary)' }}>Body</label>
                <p className="text-xs text-gray-400 mb-2">You can use basic HTML for formatting</p>
                <textarea
                  value={form.body}
                  onChange={e => setForm({ ...form, body: e.target.value })}
                  placeholder={siteConfig.marketing.exampleBody}
                  rows={12}
          ...
          {/* Preview */}
          <div className="bg-white rounded-2xl border border-gray-200 p-6">
            <h2 className="text-lg font-bold mb-6" style={{ color: 'var(--brand-primary)' }}>
            ...
            {preview ? (
              <div className="border border-gray-200 rounded-xl overflow-hidden">
                {/* Email header */}
                <div style={{ backgroundColor: 'var(--brand-primary)' }} className="px-6 py-4 text-center">
                ...
                {/* Email footer */}
                <div style={{ backgroundColor: 'var(--brand-primary)' }} className="px-6 py-3 text-center">
                  <p className="text-xs" style={{ color: 'var(--brand-muted)' }}>
                    © {new Date().getFullYear()} {siteConfig.brand.legalName}. You're receiving this because you opted in to marketing emails.
                  </p>
                </div>
              </div>
            ) : (
              <div className="space-y-4">
                ...
                <div className="flex gap-3 p-4 bg-purple-50 rounded-xl border border-purple-100">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-purple-600 mt-0.5 font-mono">{siteConfig.marketing.exampleTipBody}</p>
                  </div>
                </div>
              </div>
            )}
