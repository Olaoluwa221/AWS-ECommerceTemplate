            ...
            <div className="w-80 bg-white rounded-2xl border border-gray-200 p-6 self-start sticky top-6">
              ...
              <div className="space-y-3 mb-6">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span style={{ color: 'var(--brand-primary)' }}>{formatCurrency(selectedOrder.subtotal)}</span>
                </div>
                {selectedOrder.shippingCost > 0 && (
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">Shipping</span>
                    <span style={{ color: 'var(--brand-primary)' }}>{formatCurrency(selectedOrder.shippingCost)}</span>
                  </div>
                )}
                {selectedOrder.taxAmount > 0 && (
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">Tax</span>
                    <span style={{ color: 'var(--brand-primary)' }}>{formatCurrency(selectedOrder.taxAmount)}</span>
                  </div>
                )}
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Total</span>
                  <span className="font-bold" style={{ color: 'var(--brand-accent)' }}>{formatCurrency(selectedOrder.total)}</span>
                </div>
              </div>
