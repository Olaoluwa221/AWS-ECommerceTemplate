...
import type { UserRecord, UserRole } from '../../types/domain'
...
export default function AdminUsers() {
    ...
    const handleRoleUpdate = async (userId: string | number, newRole: UserRole) => {
        ...
        try {
            ...
            if (selectedUser?.userId === userId) {
                setSelectedUser(prev => prev ? { ...prev, userType: newRole } : prev)
            }
        } catch {
    ...
    const filtered = users.filter(u => {
        const matchesSearch =
            ...
            `${u.firstName ?? ''} ${u.lastName ?? ''}`.trim().toLowerCase().includes(search.toLowerCase())
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-6">
            <div className="max-w-7xl mx-auto">
                ...
                {/* Header */}
                <div className="mb-8">
                    <h1 className="text-3xl font-bold" style={{ color: 'var(--brand-primary)' }}>Users</h1>
                ...
                <div className="flex flex-col sm:flex-row gap-4 mb-6">
                    ...
                    <div className="flex gap-2">
                        {(['all', 'customer', 'admin'] as Array<'all' | UserRole>).map(role => (
                            <button
                                key={role}
                                ...
                                style={filterRole === role ? { backgroundColor: 'var(--brand-accent)' } : {}}
                            >
                                {role}
                            </button>
                        ))}
                    ...
                    <div className="flex-1 bg-white rounded-2xl border border-gray-200 overflow-hidden">
                        {loading ? (
                            ...
                            <div className="p-16 text-center">
                                ...
                                <h3 className="text-lg font-semibold mb-2" style={{ color: 'var(--brand-primary)' }}>No users found</h3>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex justify-end">
                                                    <select
                                                        ...
                                                        onChange={e => {
                                                            e.stopPropagation()
                                                            handleRoleUpdate(u.userId, e.target.value as UserRole)
                                                        }}
                    ...
                    {selectedUser && (
                        <div className="w-72 bg-white rounded-2xl border border-gray-200 p-6 self-start sticky top-6">
                            <div className="flex items-center justify-between mb-4">
                                <h2 className="font-bold text-lg" style={{ color: 'var(--brand-primary)' }}>User details</h2>
                            ...
                            <div className="space-y-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-medium" style={{ color: 'var(--brand-primary)' }}>{selectedUser.email}</p>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-medium" style={{ color: 'var(--brand-primary)' }}>{selectedUser.orderCount}</p>
                                </div>
                                <div>
